...
bool is_number_associated_with_a_student(const std::map< std::string, Student* >& student_numbers, const std::string& number)
{
    for(auto student: student_numbers)
    {
        if(student.second->student_number == number)
        {
            return true;
        }
    }
...
bool write_data(const std::string& file_name, std::map< std::string, Student* >& user_ids) {
    ...
    for(auto& pair: user_ids) {
        file << pair.second->student_number << ";" << pair.second->user_id << ";" << pair.second->name << ";" << pair.second->phone_number << ";" << pair.second->email << ";" << pair.second->skype << std::endl;
    }
...
void change_phone_number(Student& s1, const std::string& number, std::map< std::string, Student* >& user_ids, const std::string& file_name) {
    s1.phone_number = number; 
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            {
                if(!is_number_associated_with_a_student(student_numbers, parts.at(1)))
                {
                ...
                for(auto& pair: user_ids) {
                    if((pair.second->student_number) == parts.at(1)) {
                        change_phone_number(*(pair.second), number, user_ids, file_name);
                    }
                }
            }
