...
string convert_to_upper(const string& str)
{
    string res = "";
...
bool is_command_valid_and_parameters_correct(string& command, const vector<string>& parameters)
{
    command = convert_to_upper(command);

    for(auto c: COMMANDS)
    {
        if(c.str == command)
        {
            if(parameters.size() == c.parameter_number + 1)
            {
                return true;
            }
...
bool is_quit_command(string& command)
{
    command = convert_to_upper(command);

    for(auto c: COMMANDS)
    {
        if(c.str == command && c.is_exit)
        {
            return true;
        }
    }
...
bool are_parameters_convertable(const vector<string>& parameters, vector<double>& converted_parameters)
{
    for(size_t i = 1; i < parameters.size(); i++)
    {
        double p;
        if(!string_to_double(parameters[i], p))
        {
            cout << "Error: a non-number operand." << endl;
            return false;
        }

        converted_parameters.push_back(p);
    }

    return true;
}

void execute_command(string& command, const vector<double>& parameters)
{
    command = convert_to_upper(command);
    ...
    for(auto c: COMMANDS)
    {
        if(c.str == command)
        {
            cout << c.action(parameters[0], parameters[1]) << endl;
        }
    }
}
...
int main() {
    ...
    while ( true ) {
        ...
        if(!is_command_valid_and_parameters_correct(command_to_be_executed, pieces))
        {
            continue;
        }

        if(is_quit_command(command_to_be_executed))
        {
            cout << GREETING_AT_END << endl;
            break;
        }

        vector<double> converted_parameters;
        if(!are_parameters_convertable(pieces, converted_parameters))
        {
            continue;
        }

        execute_command(command_to_be_executed, converted_parameters);
    }
}
